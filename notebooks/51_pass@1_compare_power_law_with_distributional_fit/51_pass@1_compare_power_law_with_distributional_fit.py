...
llmonkeys_pythia_math_simulated_neg_log_avg_pass_at_k = (
    src.analyze.simulate_neg_log_avg_pass_at_k_from_beta_binomial_mle_df(
        llmonkeys_pythia_math_beta_binomial_mle_df,
        columns_to_save=["Model", "Benchmark"],
        k_values=np.unique(np.logspace(0, 4, 30, dtype=int)),
    )
)
llmonkeys_pythia_math_simulated_neg_log_avg_pass_at_k["Data"] = "Simulated"

llmonkeys_pythia_math_neg_log_avg_combined_pass_at_k_df = pd.concat(
    [
        llmonkeys_pythia_math_neg_log_avg_pass_at_k_df,
        llmonkeys_pythia_math_simulated_neg_log_avg_pass_at_k,
    ],
...
g = sns.lineplot(
    llmonkeys_pythia_math_neg_log_avg_combined_pass_at_k_df,
    x="Scaling Parameter",
    y="Neg Log Score",
    hue="Model",
...
src.plot.save_plot_with_multiple_extensions(
    ...
    plot_filename="llmonkeys_y=neg_log_score_x=scaling_parameter_hue=model_style=real_or_syn",
)
# plt.show()
...
bon_jailbreaking_neg_log_avg_pass_at_k_df["Data"] = "Real"
...
bon_jailbreaking_simulated_neg_log_avg_pass_at_k = (
    src.analyze.simulate_neg_log_avg_pass_at_k_from_beta_binomial_mle_df(
        ...
        columns_to_save=["Model", "Modality"],
        k_values=np.unique(np.logspace(0, 4, 30, dtype=int)),
    )
)
...
src.plot.save_plot_with_multiple_extensions(
    plot_dir=results_dir,
    plot_filename="bon_jailbreaking_y=neg_log_score_x=scaling_parameter_hue=model_style=real_or_syn",
)
# plt.show()
