...
g = sns.lineplot(
    data=bon_jailbreaking_vision_pass_at_k_df,
    x="Scaling Parameter",
    y="Score",
    hue="Model",
    hue_order=src.globals.BON_JAILBREAKING_VISION_MODELS_ORDER,
    style="Modality",
)
g.set(
...
bon_jailbreaking_neg_log_avg_pass_at_k_df = (
    bon_jailbreaking_vision_pass_at_k_df.groupby(
...
g = sns.relplot(
    ...
    style="Modality",
    col="Model",
    col_order=src.globals.BON_JAILBREAKING_VISION_MODELS_ORDER,
    col_wrap=4,
    estimator=None,
)
g.set(
...
for ax, model in zip(g.axes.flat, src.globals.BON_JAILBREAKING_VISION_MODELS_ORDER):
...
smallest_nonzero_pass_at_1 = bon_jailbreaking_vision_pass_at_k_df[
    bon_jailbreaking_vision_pass_at_k_df["Score"] > 0.0
]["Score"].min()
...
g = sns.displot(
    data=bon_jailbreaking_vision_pass_at_k_df[
        bon_jailbreaking_vision_pass_at_k_df["Scaling Parameter"] == 1
    ],
    kind="hist",
    x="Score",
    hue="Model",
...
# Move legend to the empty subplot position
g._legend.set_bbox_to_anchor((0.95, 0.25))
g.fig.suptitle("Best-of-N Jailbreaking (Modality = Vision)")
g.fig.subplots_adjust(top=0.9)
src.plot.save_plot_with_multiple_extensions(
    plot_dir=results_dir,
...
print("Finished notebooks/06_bon_jailbreaking_vision_eda!")
