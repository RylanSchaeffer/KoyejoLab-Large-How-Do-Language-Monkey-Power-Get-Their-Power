...
scaling_exponents_df = (
    src.analyze.create_or_load_synthetic_scaling_coefficient_data_df()
)


plt.close()
g = sns.relplot(
    data=scaling_exponents_df,
    kind="line",
    x=r"Num. Samples per Problem ($n$)",
    y=r"$(\beta - \hat{\beta})^2$",
    col="Distribution Parameters",
    hue="Num. Problems",
    hue_norm=LogNorm(),
    palette="cool",
    style="Fit Method",
...
g.set(
    ...
    ylabel=r"$(\beta - \hat{\beta})^2$",
)
g.set_titles(col_template="{col_name}")
sns.move_legend(g, "upper left", bbox_to_anchor=(1, 1.04))
src.plot.save_plot_with_multiple_extensions(
    plot_dir=results_dir,
    plot_filename="y=squared_error_x=n_hue=distribution_params_col=distribution",
)
plt.show()
