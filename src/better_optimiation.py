...
def compute_beta_binomial_three_parameter_nll_single(params, num_samples, num_successes):
    ...
    for i in range(0, n-k+1):
        ...
        log_prob_terms.append((k+i)*np.log(theta) + gammaln(alpha + beta)- gammaln(alpha+beta+i+k) + gammaln(i+k+alpha) - gammaln(alpha) + log_n_c_i)
    log_prob_terms = np.array(log_prob_terms)
    scalars = np.array([(-1)**i for i in range(n - k + 1)])
    ret = scaled_log_sum_exp(scalars, log_prob_terms) #+ gammaln(n+1) -gammaln(k+1) - gammaln(n-k+1)
    return -ret

def compute_beta_binomial_three_parameter_nll_stable(params, num_samples, num_successes):
...
def initialize_parameters_with_spread(
    ...
    if n_distr == 1:
        ...
        for i, target_mean in enumerate(target_means):
            ...
            target_var = min(target_mean * (1 - target_mean) * 0.1, 
        ...
        bounds = (
            ...
            [(0.01, 100.0) for _ in range(n_distr)] +     # betas
            [(0.01, 0.98) for _ in range(n_distr-1)]      # weights (must be < 1)
        )
...
def fit_beta_binomial_three_parameters_to_num_samples_and_num_successes(
    ...
    for i in range(n_distr):
        ...
        print(f"  Component {i+1}: α={alphas[i]:.3f}, β={betas[i]:.3f}, π={weight_i:.3f}, mean={mean_i:.3f}")
...
def compute_mixture_log_pmf(
    ...
    for i in range(n_distr):
        component_log_pmf = (
            np.log(weights[i]) + 
            scipy.stats.betabinom.logpmf(k=k, n=n, a=alphas[i], b=betas[i])
        )
