...
def fit_beta_binomial_mixture_em(num_samples, num_successes, n_components=2, max_iter=100, tol=1e-6):
...
def compute_beta_binomial_log_prob(k, n, alpha, beta):
    ...
    log_prob = scipy.special.gammaln(n + 1) - scipy.special.gammaln(k + 1) - scipy.special.gammaln(n - k + 1)
...
def compute_expected_pass_at_k(beta_mixture_params, n_distr, k1=100, k2=900):
    ...
    for i in range(n_distr):
        ...
        # Compute E[(1-p)^k1]
        term1 = np.exp(
            scipy.special.gammaln(beta_i + k1) + scipy.special.gammaln(alpha_i + beta_i) -
            scipy.special.gammaln(beta_i) - scipy.special.gammaln(alpha_i + beta_i + k1)
        )
        
        # Compute E[(1-p)^(k1+k2)]  
        term2 = np.exp(
            scipy.special.gammaln(beta_i + k1 + k2) + scipy.special.gammaln(alpha_i + beta_i) -
            scipy.special.gammaln(beta_i) - scipy.special.gammaln(alpha_i + beta_i + k1 + k2)
        )
...
def compute_scaled_expected_pass_at_k(params, k1=100, k2=900):
    ...
    # Compute E[(1-p)^k1]
    term1 = np.exp(
        -1*analyze.compute_beta_binomial_three_parameters_distribution_neg_log_likelihood((params['alpha'], params['beta']), scale = params['scale'], num_samples = np.array([k1]), num_successes=np.array([0]))
    )
    
    # Compute E[(1-p)^(k1+k2)]  
    term2 = np.exp(
        -1*analyze.compute_beta_binomial_three_parameters_distribution_neg_log_likelihood((params['alpha'], params['beta']), scale = params['scale'], num_samples = np.array([k1+k2]), num_successes=np.array([0]))
    )
    #print(term1, term2)
...
def compute_estimate(params, k):
    return 1-np.exp(-1*analyze.compute_beta_binomial_three_parameters_distribution_neg_log_likelihood(params = (params['alpha'], params['beta']), scale = params['scale'], num_samples = np.array([k]), num_successes = np.array([0]) ))

def compute_estimate_stable(params, k):
    return 1-np.exp(-1*bopt.compute_beta_binomial_three_parameter_nll_single((params['alpha'], params['beta'], params['scale']), k, 0))

def compute_p_at_ks(data, ks):
    pass_at_ks = analyze.compute_pass_at_k_from_num_samples_and_num_successes_df(data, ks)
