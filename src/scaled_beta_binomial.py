...
class scaled_beta_binom_mixture:
    def __init__(self, n_distr, num_successes, num_trials):
        ...
        n = len(num_trials)
        self.scales = [(n+1)/n for _ in range(n_distr)]
    ...
    def E_step(self):
        ...
        for i in range(self.n_distr):
            alpha = self.alphas[i]
            beta = self.betas[i]
            log_likelihoods.append(compute_beta_binomial_log_prob(self.num_successes, self.num_trials, alpha, beta) + np.log(self.probs[i]))  # Changed: add logs instead of multiply
    ...
    def single_beta_likelihood(self, params, i):
        alpha, beta = params
        log_probs = compute_beta_binomial_log_prob(self.num_successes, self.num_trials, alpha, beta)
