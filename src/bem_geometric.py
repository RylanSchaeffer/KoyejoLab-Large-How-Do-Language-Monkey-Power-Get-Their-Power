...
class beta_geometric_mixture:
    def __init__(self, n_distr, num_successes, num_trials):
        ...
        self.T_ij = np.stack([np.ones(len(num_successes))*self.probs[i] for i in range(n_distr)])
    ...
    def E_step(self):
        ...
        for i in range(self.n_distr):
            alpha = self.alphas[i]
            beta = self.betas[i]
            log_likelihoods.append(compute_beta_geometric_log_prob(self.num_successes, self.num_trials, alpha, beta) + np.log(self.probs[i]))  # Changed: add logs instead of multiply

        log_likelihoods = np.stack(log_likelihoods)    
        #use bayes' theorem with log-sum-exp for numerical stability:
        log_sum = scipy.special.logsumexp(log_likelihoods, axis=0)  # Changed: use logsumexp
    ...
    def M_step(self):
        ...
        for i in range(self.n_distr):
            self.probs[i] = self.T_ij[i].sum() / len(self.num_successes)  # Changed: closed form update
    ...
    def single_beta_likelihood(self, params, i):
        alpha, beta = params
        log_probs = compute_beta_geometric_log_prob(self.num_successes, self.num_trials, alpha, beta)
        weighted_log_likelihood = (self.T_ij[i] * log_probs).sum()  # Changed: sum instead of mean
...
def compute_estimates_better_mixture_geometric(data, params, k, n_distr):
    ...
    for row in data.iterrows():
        ...
        if (row['Num. Samples Correct'] == 0) and (row['Num. Samples Total'] < k):
            denom = 1-EM.compute_expected_pass_at_k(params, n_distr, 0, row['Num. Samples Total'])
...
def compute_estimates_better_three_param_geometric(data, params, k):
    ...
    for row in data.iterrows():
        ...
        if row['Num. Samples Correct'] == 0:
            denom = 1-EM.compute_scaled_expected_pass_at_k(params, 0, row['Num. Samples Total'])
            estimate = EM.compute_scaled_expected_pass_at_k(params, row['Num. Samples Total'], k-row['Num. Samples Total'])
            total += estimate/denom
    fail_first_false = len(data[data['Num. Samples Correct'] != 0])
    questions = len(data)
...
def compute_estimates_three_param(data, params, k):
    expected_p_at_k = np.exp(-1*analyze.compute_beta_binomial_three_parameters_distribution_neg_log_likelihood((params['alpha'], params['beta']), scale = params['scale'], num_samples = np.array([k]), num_successes = np.array([0])))
