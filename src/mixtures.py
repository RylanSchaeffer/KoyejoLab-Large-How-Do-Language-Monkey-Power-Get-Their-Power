...
def compute_beta_binomial_two_parameters_mixture_negative_log_likelihood(
    ...
    for i in range(n_distr):
        prob = np.log(params_dict[f'pi_{i}']) if i != n_distr-1 else np.log(remaining_mass)
        log_pmf = scipy.stats.betabinom.logpmf(
            k=num_successes, n=num_samples, a=params_dict[f'alpha_{i}'], b=params_dict[f'beta_{i}']
        ) + prob
...
def fit_beta_binomial_three_parameters_to_num_samples_and_num_successes(
    ...
    initial_params = [alpha for _ in range(n_distr)] + [beta for _ in range(n_distr-1)] + [initial_probs for _ in range(n_distr)]
    initial_params = tuple(initial_params)
    # Create extremely generous bounds for alpha, beta.
    bounds = [
        (0.01, 100) for _ in range(2*n_distr)
...
def plot_beta_binomial_mixture_with_histogram(
    ...
    ax1.hist(success_rates, bins=bins, density=True, alpha=alpha_hist, 
             color='skyblue', edgecolor='black', label='Observed Success Rates')
    ...
    for i in range(n_distr):
        ...
        if show_components:
            ax1.plot(x, pis[i] * component_pdf, '--', color=colors[i], 
                    alpha=0.7, label=f'Component {i+1} (π={pis[i]:.3f})')
    ...
    ax2.hist(num_successes, bins=success_bins, density=True, alpha=alpha_hist,
             color='lightcoral', edgecolor='black', label='Observed Success Counts')
    ...
    for i in range(n_distr):
        component_pmf = scipy.stats.betabinom.pmf(k_values, n_common, alphas[i], betas[i])
        mixture_pmf += pis[i] * component_pmf
        
        if show_components and n_common <= 50:  # Only show components for reasonable n
            ax2.plot(k_values, pis[i] * component_pmf, 'o--', color=colors[i],
                    alpha=0.7, markersize=3, label=f'Component {i+1} PMF')
    
    if n_common <= 50:  # Only plot PMF points for reasonable n
        ax2.plot(k_values, mixture_pmf, 'ro-', linewidth=2, markersize=4,
                label=f'Fitted Mixture PMF (n={n_common})')
    
    ax2.set_xlabel('Number of Successes')
    ax2.set_ylabel('Probability Mass')
    ax2.set_title(f'Success Counts: Histogram vs Fitted Beta-Binomial Mixture (n={n_common})')
    ax2.legend()
